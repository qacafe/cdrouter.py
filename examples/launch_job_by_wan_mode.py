...
packages = cdr.packages.list(filter=['tags@>{'+tag_name+'}'], limit='none')
...
for p in packages:
    ...
    orig = cdr.configs.get_testvar(p.config_id, name='wanMode').value

    # loop over each wanMode value
    for v in wan_modes:
        # set wanMode in package's config to v
        print '   Setting wanMode to {0}'.format(v)
        cdr.configs.edit_testvar(p.config_id, Testvar(name='wanMode', value=v))

        # launch the package with wanMode=v
        print '   Launching package'
        j = cdr.jobs.launch(Job(package_id=p.id))
    ...
    print ''
    cdr.configs.edit_testvar(p.config_id, Testvar(name='wanMode', value=orig))
