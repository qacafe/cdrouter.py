...
class TestSystem:
    ...
    def test_manual_update_license(self, cdrouter, copy_file_from_container):
        ...
        path = copy_file_from_container(container, '/usr/cdrouter-data/etc/cdrouter-*.lic')
        with open(path, 'rb') as fd: # pylint: disable=unspecified-encoding
    ...
    def test_in_use_interfaces(self, c):
        ...
        assert intfs[0].driver is None or isinstance(intfs[0].driver, str)
        assert intfs[0].firmware is None or isinstance(intfs[0].firmware, str)
        assert intfs[0].ethernet_speeds is None or isinstance(intfs[0].ethernet_speeds, list)
        assert intfs[0].wifi_modes is None or isinstance(intfs[0].wifi_modes, list)
        assert intfs[0].wifi_bands is None or isinstance(intfs[0].wifi_bands, list)
        assert intfs[0].max_clients is None or isinstance(intfs[0].max_clients, int)
